...
def sim_jaccard(prefs, genre1, genre2):  # Jaccard Distance (A, B) = |A intersection B| / |A union B|
    ...
    for item in prefs[genre1]:
        p1_union_p2[item] = 1
    #p1_union_p2 = prefs[genre1] -- WRONG !!!
    ...
    p1_intersect_p2_len, p1_union_p2_len = len(p1_intersect_p2), len(p1_union_p2)

    return float(p1_intersect_p2_len) / float(p1_union_p2_len) # return jaccard distance
...
def topMatches(prefs, person, n=5, similarity=sim_pearson):
    ...
    scores.reverse()
    return scores[0:n]
print topMatches(critics,'Michael Phillips')
